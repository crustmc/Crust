...
impl NbtType {
                        ...
                        1 => {
                            ...
                            for byte in values {
                                if let Some(byte) = byte.as_i64() {
                                    ...
                                    return Err(IOError::new(ErrorKind::InvalidData, "not a i8 in i8 array"));
                                }
                            }
                        ...
                        3 => {
                            ...
                            for byte in values {
                                if let Some(byte) = byte.as_i64() {
                                    ...
                                    return Err(IOError::new(ErrorKind::InvalidData, "not a i32 in i32 array"));
                                }
                            }
                        ...
                        12 => {
                            ...
                            for byte in values {
                                if let Some(byte) = byte.as_i64() {
                                    ...
                                    return Err(IOError::new(ErrorKind::InvalidData, "not a i64 in i64 array"));
                                }
                            }
        ...
        Self: Sized,
    {
        Ok(match id {
            ...
            _ => {
                return Err(IOError::new(ErrorKind::InvalidData, format!("unknown nbt type {id}")));
            }
        })
    }
...
pub fn read_networking_nbt<R: ReadBytesExt + ?Sized>(input: &mut R, version: i32) -> IOResult<Either<Option<NbtType>, NamedTag>> {
    ...
    if version >= version::R1_20_2 {
        Ok(Either::Left(Some(NbtType::read(input, &mut counter, tag_type)?)))
    } else {
        counter.account_bytes(28)?;
