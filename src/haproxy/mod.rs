...
impl HAProxyMessage {
    ...
    pub async fn decode_async<R: AsyncRead + Unpin + ?Sized>(source: &mut R) -> IOResult<Self> {
        ...
        if buf.as_slice() == VERSION_1_MAGIC {
            if source.read_u8().await? != b' ' {
                return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV1 protocol: missing space after magic"));
            }
            ...
            if index.is_none() {
                return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV1 protocol: missing CRLF"));
            }
            ...
            if split.len() != 5 {
                return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV1 protocol: invalid number of fields"));
            }
            ...
            return Ok(Self::V1(HAProxyMessageV1 {
                protocol_family: match proto_type {
                    "TCP4" => HAProxyProtocolFamily::TCP4 {
                        src: SocketAddrV4::new(src_ip.parse().map_err(|_| IOError::new(IOErrorKind::InvalidData, "Invalid source ip"))?, src_port.parse().map_err(|_| IOError::new(IOErrorKind::InvalidData, "Invalid source port"))?),
                        dst: SocketAddrV4::new(src_ip.parse().map_err(|_| IOError::new(IOErrorKind::InvalidData, "Invalid source ip"))?, src_port.parse().map_err(|_| IOError::new(IOErrorKind::InvalidData, "Invalid source port"))?)
                    },
                    "TCP6" => HAProxyProtocolFamily::TCP6 {
                        src: SocketAddrV6::new(dst_ip.parse().map_err(|_| IOError::new(IOErrorKind::InvalidData, "Invalid source ip"))?, dst_port.parse().map_err(|_| IOError::new(IOErrorKind::InvalidData, "Invalid source port"))?, 0, 0),
                        dst: SocketAddrV6::new(dst_ip.parse().map_err(|_| IOError::new(IOErrorKind::InvalidData, "Invalid source ip"))?, dst_port.parse().map_err(|_| IOError::new(IOErrorKind::InvalidData, "Invalid source port"))?, 0, 0)
                    },
                    _ => return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV1 protocol: invalid protocol type")),
                }
            }));
        }
        let mut buf2 = [0u8; 12];
        ...
        if buf2.as_slice() == VERSION_2_MAGIC {
            ...
            let version = ver >> 4;
            if version != 2 {
                return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV2 version"));
            }
            let command = match ver & 0xF {
                0 => HAProxyCommand::Local,
                1 => HAProxyCommand::Proxy,
                _ => return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV2 command")),
            };
            ...
            let address_family = match fam >> 4 {
                ...
                _ => return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV2 address family")),
            };
            let transport_protocol = match fam & 0xF {
                ...
                _ => return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV2 transport protocol")),
            };
            ...
            let addresses = match address_family {
                HAPRoxyAddressFamily::Inet => match transport_protocol {
                    HAProxyTransportProtocol::Stream | HAProxyTransportProtocol::Dgram => {
                        if len < 12 {
                            return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV2 address length for TCP/UDP over IPv4"));
                        }
                        len -= 12;
                        let mut src = [0u8; 4];
                    ...
                    HAProxyTransportProtocol::Unspec => return Err(IOError::new(IOErrorKind::InvalidData, "Unexpected HAProxyV2 transport protocol: UNSPEC")),
                },
                ...
                HAPRoxyAddressFamily::Unix => {
                    if len < 216 {
                        return Err(IOError::new(IOErrorKind::InvalidData, "Invalid HAProxyV2 address length for UNIX"));
                    }
                    len -= 216;
                    let mut src = [0u8; 108];
                    source.read_exact(&mut src).await?;
                    let mut dst = [0u8; 108];
                    source.read_exact(&mut dst).await?;
                    let src = String::from_utf8_lossy(&src.into_iter().take_while(|&c| c != 0).collect::<Vec<_>>()).to_string();
                    let dst = String::from_utf8_lossy(&dst.into_iter().take_while(|&c| c != 0).collect::<Vec<_>>()).to_string();
            ...
            return Ok(HAProxyMessage::V2(HAProxyMessageV2 { command, address_family, transport_protocol, addresses }));
        }
        Err(IOError::new(IOErrorKind::InvalidData, "Failed to identify HAProxy protocol version"))
    }
    ...
    pub async fn encode_async<W: AsyncWrite + Unpin + ?Sized>(&self, dest: &mut W) -> IOResult<()> {
        match self {
            ...
            Self::V2(HAProxyMessageV2 { command, address_family, transport_protocol, addresses }) => {
                dest.write_all(VERSION_2_MAGIC).await?;
                dest.write_u8((2 << 4) | *command as u8).await?;
                dest.write_u8((*address_family as u8) << 4 | (*transport_protocol as u8)).await?;
                match addresses {
                    HAProxyAdresses::Inet { src, dst } => {
    ...
    async fn write_padded_string<W: AsyncWrite + Unpin + ?Sized>(dest: &mut W, s: &str, len: usize) -> IOResult<()> {
...
pub enum HAProxyProtocolFamily {
    TCP4 { src: SocketAddrV4, dst: SocketAddrV4 },
    TCP6 { src: SocketAddrV6, dst: SocketAddrV6 },
    Unknown,
}
...
pub enum HAProxyAdresses {
    Inet { src: SocketAddrV4, dst: SocketAddrV4 },
    Inet6 { src: SocketAddrV6, dst: SocketAddrV6 },
